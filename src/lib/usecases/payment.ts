...
export async function createPayment(sql: Sql, deps: { payment: PaymentProvider; clock: Clock }, input: { token: string; returnUrl: string }): Promise<{ paymentId: number; payUrl: string }> {
  ...
  const [b] = await sql<{ id: number; status: BookingStatus; holdUntil: Date | null; payDeadline: Date | null; service: { title: string; prepayKopecks: number }; email: string }[]>`
    select b.id, b.status, b.hold_until, b.pay_deadline, b.service, p.email from bookings b join patients p on p.id = b.patient_id where b.token = ${input.token}`;
  if (!b) throw new UsecaseError("not_found", "запись не найдена");
  // Платить можно, пока окно удержано (онлайн сразу) или не истёк срок брони.
  const payable = (b.status === "held" && b.holdUntil != null && b.holdUntil > now)
    || ((b.status === "pending" || b.status === "claimed") && b.payDeadline != null && b.payDeadline > now);
  if (!payable) throw new UsecaseError("bad_status", "срок оплаты истёк или запись уже не ждёт оплаты");
