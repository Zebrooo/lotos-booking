...
A(Paragraph("Онлайн-запись: одно решение и четыре факта", title))
...
A(Paragraph(
    ...
    "и четыре изменения в замысле, которых требует закон.", body))
...
A(Paragraph("Четыре вещи, которые придётся изменить", h))
...
A(Paragraph("<b>2. «Предоплата не возвращается» писать нельзя.</b> Пациент вправе отказаться от "
            "услуги в любое время, вы это и сами отметили в ответах. В правилах пишем о возмещении "
            "фактических расходов, на практике возвращаем по требованию, как сейчас.", item))
A(Paragraph("<b>3. Запретить отмену в программе нельзя.</b> Кнопка отмены работает всегда, включая "
            "последние сутки. Дисциплину держим иначе: предупреждение до оплаты, отдельная галочка "
            "согласия, учёт поздних отмен по каждому пациенту.", item))
A(Paragraph("<b>4. «При неявке предоплата отдаётся врачу» надо перестроить.</b> Пациент может "
            "потребовать деньги назад и после неявки, а они уже отданы. Деньги остаются в клинике, "
            "врач получает премию той же суммы. Для врача ничего не меняется.", item))
...
A(Paragraph("Ваше решение по первому пункту. Юрист для текстов: договор-оферта, политика обработки "
            "данных, два отдельных согласия. Один ответственный администратор, с которым мы будем "
            "работать.", body))
...
doc = SimpleDocTemplate(str(OUT), pagesize=A4, leftMargin=MARGIN, rightMargin=MARGIN,
                        ...
                        title="Онлайн-запись «Лотос»: одно решение и четыре факта")
